<template>
  <div>
    <home-title></home-title>
    <div class="banner" text="center #fff" h-600 pt-100>
      <div class="title" text="96" font="700">规范严谨 保障权益</div>
      <div text="36" flex justify-center gap-20>
        <div class="line-left"></div>
        <div class="sub">法律保障</div>
        <div class="line-right"></div>
      </div>
    </div>
    <div class="block1" pt-60 pb-80 bg="#F4F7F9">
      <div class="title" text="center 48" font-700 mb-40>
        开立交易账户之前 请了解相关法律文件
      </div>
      <div class="wrapper" flex flex-col items-center gap-40>
        <div
          class="item"
          w-1440
          h-200
          bg="white"
          px-40
          rounded-16
          flex
          items-center
          relative
        >
          <img w-105 h120 src="@/assets/images/law/block1-icon1.png" />
          <div class="des" text="30" font-700 ml-60>订单执行政策</div>
          <n-button
            class="btn"
            absolute
            right-70
            w-160
            h-50
            round
            color="#00A3E7"
            @click="jumpTo({ name: 'order' })"
            >查看文件</n-button
          >
        </div>
        <div
          class="item"
          w-1440
          h-200
          bg="white"
          px-40
          rounded-16
          flex
          items-center
          relative
        >
          <img w-105 h120 src="@/assets/images/law/block1-icon2.png" />
          <div class="des" text="30" font-700 ml-60>风险通知</div>
          <n-button
            class="btn"
            absolute
            right-70
            w-160
            h-50
            round
            color="#00A3E7"
            @click="jumpTo({ name: 'risk' })"
            >查看文件</n-button
          >
        </div>
        <div
          class="item"
          w-1440
          h-200
          bg="white"
          px-40
          rounded-16
          flex
          items-center
          relative
        >
          <img w-105 h120 src="@/assets/images/law/block1-icon3.png" />
          <div class="des" text="30" font-700 ml-60>服务条款</div>
          <n-button
            class="btn"
            absolute
            right-70
            w-160
            h-50
            round
            color="#00A3E7"
            @click="jumpTo({ name: 'service' })"
            >查看文件</n-button
          >
        </div>
        <div
          class="item"
          w-1440
          h-200
          bg="white"
          px-40
          rounded-16
          flex
          items-center
          relative
        >
          <img w-105 h120 src="@/assets/images/law/block1-icon4.png" />
          <div class="des" text="30" font-700 ml-60>隐私权政策</div>
          <n-button
            class="btn"
            absolute
            right-70
            w-160
            h-50
            round
            color="#00A3E7"
            @click="jumpTo({ name: 'privacy' })"
            >查看文件</n-button
          >
        </div>
        <div
          class="item"
          w-1440
          h-200
          bg="white"
          px-40
          rounded-16
          flex
          items-center
          relative
        >
          <img w-105 h120 src="@/assets/images/law/block1-icon5.png" />
          <div class="des" text="30" font-700 ml-60>Cookies政策</div>
          <n-button
            class="btn"
            absolute
            right-70
            w-160
            h-50
            round
            color="#00A3E7"
            @click="jumpTo({ name: 'cookies' })"
            >查看文件</n-button
          >
        </div>
      </div>
    </div>
    <div class="block2" h-600 pt-230 text="center 48 white" font-700>
      我们提供完整规范的法律文件，确保每项都有法可依、有据可循，<br
        v-if="!isMobile"
      />
      为你的投资交易提供坚实的法律保障
    </div>
    <div class="advantage6" py-80 bg="#F5F7F9">
      <div class="wrapper" flex justify-center gap-40 rounded-10 text="#fff">
        <div
          class="description-item"
          flex
          w-256
          h-100
          gap-20
          items-center
          bg-white
          px-20
          rounded-10
        >
          <img
            h-60
            w-60
            src="@/assets/images/advantage/advantage6-icon1.png"
            alt=""
          />
          <div>
            <div class="main">100%</div>
            <div class="sub">监管合规</div>
          </div>
        </div>
        <div
          class="description-item"
          flex
          w-256
          h-100
          gap-20
          items-center
          px-20
          rounded-8
        >
          <img
            h-60
            w-60
            src="@/assets/images/advantage/advantage6-icon2.png"
            alt=""
          />
          <div>
            <div class="main">$0</div>
            <div class="sub">0手续费</div>
          </div>
        </div>
        <div
          class="description-item"
          flex
          w-256
          h-100
          gap-20
          items-center
          px-20
          rounded-8
        >
          <img h-60 w-60 src="@/assets/images/advantage/advantage6-icon3.png" />
          <div>
            <div class="main">24小时</div>
            <div class="sub">客服支持</div>
          </div>
        </div>
        <div
          class="description-item"
          flex
          w-256
          h-100
          gap-20
          items-center
          px-20
          rounded-8
        >
          <img h-60 w-60 src="@/assets/images/advantage/advantage6-icon4.png" />
          <div>
            <div class="main">17年</div>
            <div class="sub">以上经验</div>
          </div>
        </div>
        <div
          class="description-item"
          flex
          w-256
          h-100
          gap-20
          items-center
          px-20
          rounded-8
        >
          <img h-60 w-60 src="@/assets/images/advantage/advantage6-icon5.png" />
          <div>
            <div class="main">极速</div>
            <div class="sub">出入金服务</div>
          </div>
        </div>
      </div>
    </div>
    <div class="advantage7" h-370 pt-60 pb-80>
      <div class="title" text="48 center #fff" font-700>
        只需三分钟开户 迈向财富的第一步
      </div>
      <div class="wrapper" relative flex justify-center gap-40 mt-40>
        <div
          class="download-item"
          bg-white
          flex
          items-center
          h-120
          w-330
          gap-20
          px-20
          rounded-12
        >
          <img w-80 src="@/assets/images/home/ios.png" alt="" />
          <div>
            <div class="main-content" font-500 text-30>App Store</div>
            <div class="sub-content" font-400 text="16 #808080">下载></div>
          </div>
        </div>
        <div
          class="download-item"
          bg-white
          flex
          items-center
          h-120
          w-330
          gap-20
          px-20
          rounded-12
        >
          <img w-80 src="@/assets/images/home/google.png" alt="" />
          <div>
            <div class="main-content" font-500 text-30>Google Play</div>
            <div class="sub-content" font-400 text="16 #808080">下载></div>
          </div>
        </div>
        <div
          class="download-item"
          bg-white
          flex
          items-center
          h-120
          w-330
          gap-20
          px-20
          rounded-12
        >
          <img w-80 src="@/assets/images/home/person-1.png" alt="" />
          <div>
            <div class="main-content" font-500 text-30>真实账户</div>
            <div class="sub-content" font-400 text="16 #808080">开设></div>
          </div>
        </div>
        <div
          class="download-item"
          bg-white
          flex
          items-center
          h-120
          w-330
          gap-20
          px-20
          rounded-12
        >
          <img w-80 src="@/assets/images/home/person-2.png" alt="" />
          <div>
            <div class="main-content" font-500 text-30>模拟账户</div>
            <div class="sub-content" font-400 text="16 #808080">开设></div>
          </div>
        </div>
      </div>
    </div>
    <home-footer></home-footer>
  </div>
</template>

<script setup lang="ts">
  import { router } from '@/router'
  import { useGlobalStore } from '@/store'
import { jumpTo } from '@/utils'
  import { storeToRefs } from 'pinia'

  const { isMobile } = storeToRefs(useGlobalStore())

</script>

<style scoped>
  @import url('./mobile.scss');
  @media screen and (min-width: 768px) {
    .banner {
      background: url('@/assets/images/law/banner.png') no-repeat center;
      background-size: cover;
      .line-right {
        border-top: 2px solid;
        margin-top: 7.5rem;
        width: 35rem;
        border-image: linear-gradient(
            90deg,
            rgba(216.00000232458115, 216.00000232458115, 216.00000232458115, 1),
            rgba(216.00000232458115, 216.00000232458115, 216.00000232458115, 0)
          )
          2 2;
      }
      .line-left {
        border-top: 2px solid;
        margin-top: 7.5rem;
        /* height: 2px; */
        width: 35rem;
        border-image: linear-gradient(
            90deg,
            rgba(216.00000232458115, 216.00000232458115, 216.00000232458115, 0),
            rgba(216.00000232458115, 216.00000232458115, 216.00000232458115, 1)
          )
          2 2;
      }
    }
    .block2 {
      background: url('@/assets/images/law/block2-bg.png') no-repeat center;
      background-size: cover;
    }
    .advantage7 {
      background: url('@/assets/images/advantage/advantage7-bg.png') no-repeat
        center;
      background-size: cover;
    }
    .description-item {
      background: linear-gradient(180deg, #2c64ff 0%, #3b81fe 100%);
    }
  }
</style>
